...
public class Game extends JPanel implements KeyListener
{
    ...
    {
        if ( e.getKeyChar() == '3' || e.getKeyCode() == KeyEvent.VK_UP )
        {
            game.up();
            game.spawn();
        ...
        else if ( e.getKeyChar() == '4' || e.getKeyCode() == KeyEvent.VK_DOWN )
        {
            game.down();
            game.spawn();
            gameBoard = game.toString();
        ...
        else if ( e.getKeyChar() == '1' || e.getKeyCode() == KeyEvent.VK_LEFT )
        {
            game.left();
            game.spawn();
            gameBoard = game.toString();
        ...
        else if ( e.getKeyChar() == '2' || e.getKeyCode() == KeyEvent.VK_RIGHT )
        {
            game.right();
            game.spawn();
            gameBoard = game.toString();
    ...
    public void paint( Graphics g )
    {
        ...
        for ( int i = 0; i < 4; i++ )
        {
            for ( int j = 0; j < 4; j++ )
            {
                drawTile( g, game.board[i][j], j * 60 + 150, i * 60 + 60 );
            }
        }
        if ( game.gameOver() )
        {
